...
input = numpy.zeros((0, 228))
testinput = numpy.zeros((0, 228))
...
input = input.reshape(len(input), 3, 76)
input = input.transpose(0, 2, 1)
print (input.shape)

testinput = testinput.reshape(len(testinput), 3, 76)
...
# Create the model
model=Sequential()
model.add(Conv1D(filters=20, kernel_size=6, padding='valid', activation='relu', strides=1, input_shape=(76, 3)))
...
hist = model.fit(input, Y_train, batch_size=32, epochs=600, verbose=1, validation_data=(testinput, Y_test), steps_per_epoch=None)		
...
model.save("D:\\model_conv1d_ov.h5")
